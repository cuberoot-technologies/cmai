<script>
	import FormMember from './FormMember.svelte';
</script>

<div>
	<div class="container mx-auto mt-5 p-4">
		<div class="grid grid-cols-1 md:grid-cols-12">
			<!-- Left Sidebar (smaller) -->
			<div class="space-y-9 md:col-span-9">
				<div class="bg-white">
					<a href="" > view more</a>
					<h2 class="text-dblue mb-4 text-2xl font-semibold">Membership Benefits

					</h2>
				</div>
				<div class="bg-light mx-auto shadow-md">
						<h2
						class="bg-dblue relative flex justify-between w-full border-b-2 border-dotted border-sky-800 px-4 py-2 text-xl font-bold text-white"
					>
						Policy Advocacy to Government and Regulators
					<a href="/policy-advocacy-to-government-and-regulators" > view more</a>

					
				</h2>
					<div class="space-y-4">
						<!-- Event 1 -->
						<div class="    mb-0 border-b-2 border-dotted border-gray-200 p-3">
							<p class="text-xl text-gray-600">1. Representations on behalf of the industry</p>
						</div>

						<!-- Event 2 -->
						<div class="   mb-0 p-3">
							<p class="text-xl text-gray-600">
								2. Get recognized as the representative body of the concerned sector among all the
								stakeholders.
							</p>
						</div>

						<!-- Event 4 -->
					</div>
				</div>

				<div class=" shadow">
					<div class="bg-light mx-auto shadow-md">
						<h2
							class="bg-dblue relative flex justify-between w-full border-b-2 border-dotted border-sky-800 px-4 py-2 text-xl font-bold text-white"
						>
							Networking Opportunities with Corporate Leaders, Government Authorities and
							International Fora
					<a href="/networking-opportunities-with-corporate-leaders" > view more</a>
						
						</h2>
						<div class="space-y-4">
							<!-- Event 1 -->
							<div class="mt-2 flex items-center justify-between p-2">
								<p class="text-base text-lg text-gray-800">➢ Call on meetings</p>
							</div>

							<!-- Event 2 -->
							<div class="flex items-center justify-between p-2">
								<p class="text-base text-lg text-gray-800">➢ Interactive sessions</p>
							</div>
						</div>
					</div>
				</div>

				<div class="bg-light mx-auto shadow-md">
					<h2
						class="bg-dblue relative flex justify-between w-full border-b-2 border-dotted border-sky-800 px-4 py-2 text-xl font-bold text-white"
					>
						Advisory Services to Business
					<a href="/regular-industry" > view more</a>
					
					</h2>
					<div class="space-y-4">
						<!-- Event 1 -->
						<div class="      mb-0 border-b-2 border-dotted border-gray-200 p-3">
							<p class="text-xl text-gray-600">
								1. Circulars on policy updates, Govt notifications, govt. schemes etc.
							</p>
						</div>

						<!-- Event 2 -->
						<div class="   mb-0 border-b-2 border-dotted border-gray-200 p-3">
							<p class="text-xl text-gray-600">
								2. Newsletters on industry updates / sectoral information
							</p>
						</div>
						<div class="   mb-0 p-3">
							<p class="text-xl text-gray-600">
								3. Share and develop robust mitigation and adaptation strategies
							</p>
						</div>

						<!-- Event 4 -->
					</div>
				</div>
				<div class="bg-light mx-auto shadow-md">
					<h2
						class="bg-dblue relative flex justify-between w-full border-b-2 border-dotted border-sky-800 px-4 py-2 text-xl font-bold text-white"
					>
						Knowledge Papers and Research Reports
					<a href="/knowledge-papers" > view more</a>
					
					</h2>
					<div class="space-y-4">
						<!-- Event 1 -->
						<div class="    mb-0 border-b-2 border-dotted border-gray-200 p-3">
							<p class="text-xl text-gray-600">1. Research Study</p>
						</div>

						<!-- Event 2 -->
						<div class="   mb-0 border-b-2 border-dotted border-gray-200 p-3">
							<p class="text-xl text-gray-600">2. Knowledge Reports</p>
						</div>
						<div class="   mb-0 border-b-2 border-dotted border-gray-200 p-3">
							<p class="text-xl text-gray-600">3. Policy Papers</p>
						</div>
						<div class="   mb-0 p-3">
							<p class="text-xl text-gray-600">4. Monthly Newsletter</p>
						</div>
					</div>
				</div>
				<div class=" shadow">
					<div class="bg-light mx-auto shadow-md">
						<h2
							class="bg-dblue relative flex justify-between w-full border-b-2 border-dotted border-sky-800 px-4 py-2 text-xl font-bold text-white"
						>
							Capacity Building Workshops and Trainings for Stakeholders
					<a href="/capacity-building-workshops" > view more</a>
						
						</h2>
						<div class="space-y-4">
							<!-- Event 1 -->
							<div class="mt-2 flex items-center justify-between p-2">
								<p class="text-base text-lg text-gray-800">➢ Conference/seminars/webinars</p>
							</div>

							<!-- Event 2 -->
							<div class="flex items-center justify-between p-2">
								<p class="text-base text-lg text-gray-800">➢ Training programs</p>
							</div>
							<div class="flex items-center justify-between p-2">
								<p class="text-base text-lg text-gray-800">➢ Exhibitions</p>
							</div>
						</div>
					</div>
				</div>

				<div class=" relative shadow">
					<div class="bg-light mx-auto shadow-md">
						<h2
							class="bg-dblue relative flex justify-between w-full border-b-2 border-dotted border-sky-800 px-4 py-2 text-xl font-bold text-white"
						>
							Other Benefits
						
						</h2>
						<div class="space-y-4">
							<!-- Event 1 -->
							<div class="mt-2 flex items-center justify-between p-2">
								<p class="text-base text-lg text-gray-800">
									➢ General Membership access of strategic partners - SAFA, ICCA & CLA
								</p>
							</div>

							<!-- Event 2 -->
							<div class="flex items-center justify-between p-2">
								<p class="text-base text-lg text-gray-800">
									➢ Branding in all the events organized by the association
								</p>
							</div>
							<div class="flex items-center justify-between p-2">
								<p class="text-base text-lg text-gray-800">
									➢ Opportunity to be the Chairperson/Co-Chairperson of the different sub-committee.
								</p>
							</div>

							<div class="flex items-center justify-between p-2">
								<p class="text-base text-lg text-gray-800">
									➢ Opportunity to be the keynote speaker in the events
								</p>
							</div>
							<div class="flex items-center justify-between p-2">
								<p class="text-base text-lg text-gray-800">
									➢ Will have special mention on the website and social media pages of CMAI
								</p>
							</div>
							
							<div class=" hidden md:block absolute right-0 bottom-0 flex items-center justify-center p-2">
								<div class="flex gap-4 text-white sm:justify-between">
									<a
										href="https://www.instagram.com/cmai_carbonmarkets/"
										target="_blank"
										class="bg-dblue rounded-lg p-2"
									>
										<svg
											xmlns="http://www.w3.org/2000/svg"
											width="18"
											height="18"
											viewBox="0 0 24 24"
											fill="currentColor"
										>
											<path
												d="M12 2.2c3.2 0 3.6 0 4.9.1 1.2.1 2 .3 2.7.7.7.3 1.3.8 1.9 1.4.5.6 1 1.2 1.4 1.9.4.7.6 1.5.7 2.7.1 1.3.1 1.7.1 4.9s0 3.6-.1 4.9c-.1 1.2-.3 2-.7 2.7-.3.7-.8 1.3-1.4 1.9-.6.5-1.2 1-1.9 1.4-.7.4-1.5.6-2.7.7-1.3.1-1.7.1-4.9.1s-3.6 0-4.9-.1c-1.2-.1-2-.3-2.7-.7-.7-.3-1.3-.8-1.9-1.4-.5-.6-1-1.2-1.4-1.9-.4-.7-.6-1.5-.7-2.7C2.2 15.6 2.2 15.2 2.2 12s0-3.6.1-4.9c.1-1.2.3-2 .7-2.7.3-.7.8-1.3 1.4-1.9.6-.5 1.2-1 1.9-1.4.7-.4 1.5-.6 2.7-.7C8.4 2.2 8.8 2.2 12 2.2m0-2.2C8.7 0 8.3 0 7 .1 5.7.2 4.6.5 3.7 1 .9 1.8 0 3.1 0 5c-.1 1.2-.1 1.6-.1 4.9s0 3.6.1 4.9c.1 1.9.9 3.2 2.7 4 .9.5 2 .8 3.3.9 1.3.1 1.7.1 4.9.1s3.6 0 4.9-.1c1.3-.1 2.4-.4 3.3-.9 1.8-.8 2.6-2.1 2.7-4 .1-1.2.1-1.6.1-4.9s0-3.6-.1-4.9c-.1-1.9-.9-3.2-2.7-4-.9-.5-2-.8-3.3-.9-1.3-.1-1.7-.1-4.9-.1zM12 5.7a6.3 6.3 0 110 12.6 6.3 6.3 0 010-12.6m0 10.4a4.1 4.1 0 100-8.2 4.1 4.1 0 000 8.2m7.3-10.9a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0z"
											/>
										</svg>
									</a>

									<a
										href="https://twitter.com/official_cmai"
										target="_blank"
										class="bg-dblue rounded-lg p-2"
									>
										<svg
											xmlns="http://www.w3.org/2000/svg"
											width="18"
											height="18"
											viewBox="0 0 24 24"
											fill="currentColor"
										>
											<path
												d="M23.64 4.56c-.84.37-1.75.61-2.7.72a4.72 4.72 0 002.08-2.59 9.36 9.36 0 01-2.97 1.13 4.67 4.67 0 00-8.01 4.26A13.28 13.28 0 011.67 3.14a4.64 4.64 0 001.45 6.22c-.74-.02-1.46-.23-2.1-.57v.06a4.66 4.66 0 003.73 4.57 4.68 4.68 0 01-2.1.08 4.67 4.67 0 004.36 3.24A9.36 9.36 0 010 20.48a13.2 13.2 0 007.18 2.1c8.6 0 13.3-7.13 13.3-13.3l-.01-.6a9.5 9.5 0 002.35-2.42z"
											/>
										</svg>
									</a>
									<a
										href="https://www.youtube.com/@OfficialCMAI"
										target="_blank"
										class="bg-dblue rounded-lg p-2"
									>
										<svg
											xmlns="http://www.w3.org/2000/svg"
											width="18"
											height="18"
											viewBox="0 0 24 24"
											fill="currentColor"
										>
											<path
												d="M23.5 6.8c-.3-1.3-1.3-2.3-2.6-2.6C18.4 3.5 12 3.5 12 3.5s-6.4 0-8.9.7A3.2 3.2 0 00.5 6.8 32.1 32.1 0 000 12c0 1.7.1 3.4.5 5.2.3 1.3 1.3 2.3 2.6 2.6 2.5.7 8.9.7 8.9.7s6.4 0 8.9-.7a3.2 3.2 0 002.6-2.6c.3-1.8.5-3.5.5-5.2s-.1-3.4-.5-5.2zm-14.3 8.6V8.6l6.2 3.4-6.2 3.4z"
											/>
										</svg>
									</a>

									<a
										href="https://www.linkedin.com/company/86816772/admin/feed/posts/"
										target="_blank"
										class="bg-dblue rounded-lg p-2"
									>
										<svg
											xmlns="http://www.w3.org/2000/svg"
											width="18"
											height="18"
											viewBox="0 0 24 24"
											fill="currentColor"
										>
											<path
												d="M22.23 0H1.77C.79 0 0 .77 0 1.72v20.56c0 .95.79 1.72 1.77 1.72h20.46c.98 0 1.77-.77 1.77-1.72V1.72C24 .77 23.21 0 22.23 0zM7.12 20.49H3.56V9h3.56v11.49zM5.34 7.56a2.06 2.06 0 110-4.13 2.06 2.06 0 010 4.13zm14.14 12.93h-3.56v-5.58c0-1.33-.03-3.04-1.85-3.04-1.85 0-2.14 1.45-2.14 2.95v5.67H8.37V9h3.42v1.57h.05c.48-.9 1.65-1.85 3.4-1.85 3.63 0 4.3 2.4 4.3 5.52v6.25z"
											/>
										</svg>
									</a>
									<a
										href="https://www.facebook.com/people/Carbon-Markets-Association-of-India/61573425383926/"
										class="bg-dblue rounded-lg p-2"
									>
										<svg
											xmlns="http://www.w3.org/2000/svg"
											width="18"
											height="18"
											viewBox="0 0 24 24"
											fill="currentColor"
										>
											<path
												d="M22.68 0H1.32C.59 0 0 .6 0 1.34v21.32C0 23.4.6 24 1.32 24h11.5v-9.3H9.65v-3.62h3.17V8.18c0-3.15 1.92-4.88 4.72-4.88 1.34 0 2.5.1 2.83.14v3.28h-1.94c-1.52 0-1.82.72-1.82 1.78v2.34h3.65l-.48 3.62h-3.17V24h6.23c.73 0 1.32-.6 1.32-1.34V1.34C24 .6 23.4 0 22.68 0z"
											/>
										</svg>
									</a>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>

			<!-- Main Content (larger) -->
			<div class="space-y-3 border-l-1 border-solid border-gray-200 pl-3 md:col-span-3">
				<div class="relative flex items-center justify-center">
					<FormMember />
					<div class=" hidden md:block absolute right-10 bottom-[-125%]">
								<img src="/images/qrbenifit.jpg" alt="" width="260px" >
							</div>
				</div>
			</div>
		</div>
	</div>
</div>

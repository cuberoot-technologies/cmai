...
<header class="relative z-10 text-white ">
	<div class=" hidden lg:block bg-stone-200">
		<div class="mx-auto flex max-w-screen-xl  justify-center items-center h-[5rem]">
				<a href="/" class="text-teal-400">
					<img src="/images/cmai.png" alt="Logo" width="170px" />
				</a>
		</div>
		<div class="flex gap-4 sm:justify-end text-black">
				...
				{#each menuItems as menu}
					<div class="relative cursor-pointer">
						<button
							...
							class="cursor-pointer text-dblue hover:text-dblue-light font-bold">{menu.name}</button
						>
						{#if (menu.key === 'about' && showAboutDropdown) || (menu.key === 'offerings' && showOfferingsDropdown) || (menu.key === 'climate' && showClimateDropdown) || (menu.key === 'knowledge' && showKnowledgeDropdown)}
							<ul
								class="absolute left-0 mt-2 w-2xs w-48 cursor-pointer rounded-md bg-white text-black shadow-lg"
							>
								{#each menu.links as link}
									<li class="cursor-pointer">
										<a href={link.path} class="block px-4 py-2 text-black hover:text-dblue-light">{link.label}</a>
									</li>
								{/each}
							</ul>
						{/if}
			...
			<ul class="space-y-2 text-center">
				{#each menuItems as menu}
					<li>
						<button
							on:click={() => toggleDropdown(menu.key)}
							class="block w-full cursor-pointer py-2 text-lg text-green-400 hover:text-green-400"
							>{menu.name}</button
						>
						{#if (menu.key === 'about' && showAboutDropdown) || (menu.key === 'offerings' && showOfferingsDropdown) || (menu.key === 'climate' && showClimateDropdown) || (menu.key === 'knowledge' && showKnowledgeDropdown)}
							<ul class="rounded-md bg-gray-700 text-sm text-gray-300">
								{#each menu.links as link}
									<li>
										<a href={link.path} class="block px-4 py-2  text-green-400 hover:bg-gray-600">{link.label}</a>
									</li>
								{/each}
							</ul>
						{/if}

<script>
	...
	const menuItems = [
		...
		},
		{
			...
			key: 'knowledge',
			links: [
				{ path: '/articles', label: 'Latest Articles' }
			]
		}
	];
</script>
...
<header class="relative z-10 text-white">
	...
	<div class="mx-auto  flex max-w-screen-xl items-center gap-2 px-6 pb-2">
		<div class="flex size-full h-16 w-full max-w-screen-xl items-center gap-2">
			<a class="block flex gap-2 text-sm text-white" href="#">
				<svg
			...
			<a class="block flex gap-2 text-sm text-white" href="#">
				...
				Working Office: 511, World Trade Centre, Connaught Place, New Delhi - 110001
			</a>
		</div>
	...
	<div class="mx-auto max-w-screen-xl px-4 py-6 sm:px-6 lg:px-8">
		<div class="flex h-16 items-center justify-between">
			...
			<nav class="hidden items-center space-x-6 md:flex ">
				{#each menuItems as menu}
					<div class="relative cursor-pointer">
						<button
							...
							class="cursor-pointer hover:text-gray-100">{menu.name}</button
						>
						{#if (menu.key === 'about' && showAboutDropdown) || (menu.key === 'offerings' && showOfferingsDropdown) || (menu.key === 'climate' && showClimateDropdown) || (menu.key === 'knowledge' && showKnowledgeDropdown)}
							<ul
								class="absolute left-0 mt-2 w-48 cursor-pointer rounded-md bg-white text-gray-900 shadow-lg w-2xs"
							>
				...
				{/each}
				<a href="#" class="cursor-pointer hover:text-gray-100">Contact Us</a>
			...
			<ul class="space-y-2 text-center">
				{#each menuItems as menu}
					<li>
						<button
							on:click={() => toggleDropdown(menu.key)}
							class="block w-full cursor-pointer py-2 text-lg text-white hover:text-gray-100"
							>{menu.name}</button
						>
						{#if (menu.key === 'about' && showAboutDropdown) || (menu.key === 'offerings' && showOfferingsDropdown) || (menu.key === 'climate' && showClimateDropdown) || (menu.key === 'knowledge' && showKnowledgeDropdown)}
				...
				{/each}
				<li>
					<a href="#" class="block cursor-pointer py-2 text-white hover:text-gray-100"
						>Contact Us</a
					>
				</li>
